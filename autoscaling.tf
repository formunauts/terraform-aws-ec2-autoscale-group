...
locals {
  autoscaling_enabled = var.enabled && var.autoscaling_policies_enabled
  name_prefix         = "${module.label.id}${var.delimiter}scale${var.delimiter}"
}

resource "aws_autoscaling_policy" "scale_up" {
  count = local.autoscaling_enabled ? 1 : 0

  name                   = "${local.name_prefix}up"
